...
public class BookModel extends DefaultTableModel {
    ...
    public String insertIntoBooksTableAndSetID(String name, String isbn, String borrow, String date, String author, String description, String price) {
        String id = null;
        ResultSet rs = null;
        String query = "INSERT INTO Books(Book_Name,Book_ISBN,Borrow_Status,BUY_DATE,author,description,price)"
                + " values(?,?,?,?,?,?,?) ";
    ...
    public void updateBooksTable(int rowInTable, String newName, String newIsbn, String newAuthor, String newDescription, String newPrice) {

        String query = "UPDATE books SET Book_Name=? , Book_ISBN=?, author=?, description=?, price=? where Book_ID=? ";
        ...
        try (Connection con = DriverManager.getConnection(dbUrl, "root", initDB.getMysqlPassword());
            ...
            ps.setObject(3, newAuthor);
            ps.setObject(4, newDescription);
            ps.setObject(5, newPrice);
            ps.setObject(6, id);
            ps.addBatch();
            if (ps.executeUpdate() == 1) {
